...
test('isRowEmpty correctly identifies white/non-white rows', () => {
  ...
  assert.strictEqual(
    detector.isRowEmpty(pixelBuffer, 0, width, channels),
    true,
    'Row 0 should be empty (white)',
  )
  assert.strictEqual(
    detector.isRowEmpty(pixelBuffer, 1, width, channels),
    false,
    'Row 1 should not be empty (grey)',
  )
  assert.strictEqual(
    detector.isRowEmpty(pixelBuffer, 2, width, channels),
    false,
    'Row 2 should not be empty (red)',
  )
})
