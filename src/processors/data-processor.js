...
export class DataProcessor {
  ...
  processExample(example) {
    ...
    // Fix p. 39-r
    processed = processed.replace(/1 Auf dem Brief fehlt der Absender/, '1. Auf dem Brief fehlt der Absender');
    ...
    if (processed.includes('Hausfrau') && processed.includes('Hausmann') && processed.includes('kümmert')) {
  ...
  applyCosmeticFixes(def) {
    return def
      ...
      .replace(/festnehmen nimmt fest, nahm fest, hat festgenommen\./, 'festnehmen, nimmt fest, nahm fest, hat festgenommen');
  }
