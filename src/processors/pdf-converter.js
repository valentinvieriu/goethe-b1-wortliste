 ...
 * using MuPDF.js (WASM).  The output filenames and DPI match the
 * original Poppler‐based implementation so that the rest of the
...
export class PDFConverter {
  ...
  /**
   * Render requested pages as PNG files (300 DPI by default).
   * Skips pages that have already been rendered.
   */
  async convertPDFToPNG() {
    ...
    // Simple heuristic: if the very last expected file exists we assume
    ...
    console.log('Converting PDF to PNG files with MuPDF.js …');
    ...
    for (let zeroBased = 0; zeroBased < pageCount; zeroBased++) {
      ...
      const page = doc.loadPage(zeroBased);
